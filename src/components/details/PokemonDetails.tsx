...
export const PokemonDetails = ({
	...
	return (
		<div className="min-h-dvh bg-paper" style={palette}>
			...
			<div className="mx-auto w-full max-w-2xl px-4 py-5 sm:px-6 lg:hidden">
				<header
			...
			<div className="relative hidden min-h-dvh flex-col lg:flex">
				...
				<div className="sticky top-0 z-30 border-b border-line/70 bg-paper/85 backdrop-blur-md">
					<div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-8 px-8 py-3.5 xl:px-12 2xl:max-w-[96rem]">
						<Link
							to="/pokemon"
							search={{ search: undefined, types: [], sort: "id", dir: "asc" }}
							className="inline-flex items-center gap-1.5 rounded-full px-2 py-1.5 font-mono text-[0.68rem] font-bold uppercase tracking-widest text-ink-soft transition hover:text-ink"
						>
				...
				{/* hero band */}
				<section
					className="grain relative overflow-hidden text-(--on-type)"
					style={{ background: heroGradient }}
				>
					<div className="dotfield pointer-events-none absolute right-12 top-12 size-44 opacity-25" />
					<div
						aria-hidden
						className="ghost-in pointer-events-none absolute -bottom-8 right-10 font-display text-[14rem] font-extrabold leading-none tracking-tight opacity-[0.08] xl:text-[18rem] 2xl:text-[22rem]"
					>
						{formatId(species.id)}
					</div>

					<div className="mx-auto grid w-full max-w-7xl grid-cols-[0.85fr_1fr] items-center gap-16 px-8 py-20 xl:gap-24 xl:px-12 xl:py-28 2xl:max-w-[96rem]">
						{/* sprite */}
						<div className="relative flex items-center justify-center">
							{sprite && (
								<div className="fade-in relative 2xl:scale-110">
									<PokeballIcon className="pointer-events-none absolute left-1/2 top-1/2 size-[28rem] -translate-x-1/2 -translate-y-1/2 text-current opacity-[0.08] [&_path]:fill-current" />
									<div
				...
				<main className="flex flex-1 flex-col bg-paper">
					<div className="mx-auto w-full max-w-7xl flex-1 px-8 py-14 xl:px-12 xl:py-16 2xl:max-w-[96rem]">
						<DetailTabs tab={tab} />
					</div>

					{/* prev / next — full-width bar at the bottom */}
					<div className="border-t border-line">
						<div className="mx-auto w-full max-w-7xl px-8 py-5 xl:px-12 2xl:max-w-[96rem]">
							<PokemonNavigation neighbors={neighbors} />
						</div>
					</div>
				</main>
